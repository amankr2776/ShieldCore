...
export default function LiveFeedPage() {
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground selection:bg-destructive/30 dashboard-cursor transition-colors">
      ...
      <div className="container mx-auto py-12 px-6 max-w-7xl space-y-10 animate-in fade-in duration-1000 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-2">
            ...
            <p className="opacity-60 font-medium text-lg italic">Edge-wide telemetry across global ingress nodes.</p>
          </div>
          <div className="flex items-center gap-4 bg-card/40 p-2 rounded-2xl border border-border/40 backdrop-blur-xl">
            <div className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-accent border border-border/40 text-[10px] font-black uppercase tracking-widest">
              <div className={cn("h-2 w-2 rounded-full", isConnected ? "bg-emerald-500 animate-pulse shadow-[0_0_10px_#22c55e]" : "bg-destructive")} />
              <span className="opacity-60">{isConnected ? "Uplink Secure" : "Uplink Lost"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: 'Total Ingress', value: stats.total, color: 'text-foreground', trend: '+12%', icon: Globe },
            { label: 'Neutralized', value: stats.blocked, color: 'text-destructive', trend: '+4%', icon: ShieldX },
            { label: 'Avg Threat Index', value: `${Math.round(stats.avgScore * 100)}%`, color: 'text-amber-600 dark:text-amber-500', trend: '-2%', icon: AlertCircle },
          ].map((s, i) => (
            <Card key={i} className="glass-card rounded-3xl overflow-hidden group p-1">
              <CardContent className="p-8 flex items-center justify-between">
                <div className="space-y-2">
                  ...
                  <div className="text-[10px] font-bold text-emerald-500">{s.trend} <span className="opacity-40">vs prev epoch</span></div>
                </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-card/20 p-6 rounded-3xl border border-border/40">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <Button variant="outline" size="lg" onClick={() => setIsPaused(!isPaused)} className={cn("h-14 px-8 font-black uppercase text-[10px] tracking-widest rounded-2xl transition-all border-border/40 hover:bg-accent", isPaused ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 border-emerald-500/20" : "bg-destructive/10 text-destructive border-destructive/20")}>
              {isPaused ? <Play className="h-4 w-4 mr-2" /> : <Pause className="h-4 w-4 mr-2" />}
              {isPaused ? "Activate Feed" : "Suspend Feed"}
            </Button>
            <div className="relative flex-1 md:w-96">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 opacity-30" />
              <Input placeholder="Search IP or Attack Class..." className="h-14 pl-12 bg-background/50 rounded-2xl border-border/40 focus-visible:ring-destructive font-mono text-xs" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="flex gap-2 bg-background/50 p-1.5 rounded-2xl border border-border/40">
                {['ALL', 'BLOCKED', 'SAFE'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} className={cn("px-6 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all", filter === f ? "bg-destructive text-white shadow-lg" : "opacity-60 hover:opacity-100 hover:bg-accent")}>
                    {f}
                  </button>
                ))}
             </div>
             <Button variant="ghost" size="icon" className="h-14 w-14 rounded-2xl border border-border/40 hover:bg-destructive/10 hover:text-destructive opacity-60" onClick={() => setRequests([])}>
                <Trash2 className="h-5 w-5" />
             </Button>
          </div>
        </div>

        <div className="space-y-4 max-h-[1200px] overflow-auto pr-2 custom-scrollbar no-scrollbar">
              ...
              >
                <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-6 flex-1 min-w-0" onClick={() => setSelectedRequest(req)}>
                    <div className={cn("p-4 rounded-2xl border border-border/40 transition-transform group-hover:scale-110 cursor-pointer", req.decision === 'BLOCKED' ? "bg-destructive/10 text-destructive" : "bg-emerald-500/10 text-emerald-500")}>
                      <Icon className="h-6 w-6" />
                    </div>
                    <div className="min-w-0 flex-1 cursor-pointer">
                      <div className="flex items-center gap-2 mb-1">
                         <span className="text-[12px] font-mono font-black">{req.ip}</span>
                  ...
                  <div className="flex flex-col items-end gap-3 w-full md:w-auto">
                    <div className="flex items-center gap-4">
                       <div className="text-right">
                          <p className="text-[9px] font-black uppercase opacity-50 mb-1">Intelligence Match</p>
                    ...
                    <div className="flex items-center gap-4 w-full md:w-auto justify-end">
                       <div className="flex items-center gap-4 text-[9px] font-black opacity-40 uppercase tracking-widest mr-4">
                       ...
                       {req.decision === 'BLOCKED' && (
                         <Button 
                            ...
                            className="h-8 border-destructive text-destructive hover:bg-destructive hover:text-white transition-all rounded-lg group px-4 font-black uppercase text-[9px]"
                         >
        ...
        <Sheet open={!!selectedRequest} onOpenChange={(open) => !open && setSelectedRequest(null)}>
          <SheetContent className="w-full sm:max-w-2xl glass-card border-l border-border/40 shadow-2xl p-0 overflow-hidden flex flex-col">
                ...
                <div className="p-12 space-y-12 flex-1 overflow-auto custom-scrollbar no-scrollbar">
                  <SheetHeader>
                    <div className="flex items-center justify-between mb-4">
                      <Badge variant="outline" className="text-[10px] font-black uppercase px-4 py-1 border-border/40">{selectedRequest.id.toUpperCase()}</Badge>
                  ...
                  <div className="space-y-10">
                    ...
                    <div className="space-y-8 relative">
                      <div className="absolute left-4 top-4 bottom-4 w-[2px] border-l-2 border-dashed border-border/20" />
                      {[
                        ...
                        <div key={idx} className={cn("pl-12 relative transition-all duration-700", idx > replayStep ? "opacity-0 translate-x-4" : "opacity-100 translate-x-0")}>
                          <div className={cn("absolute left-0 top-0 h-8 w-8 rounded-full flex items-center justify-center border-2 z-10", idx === replayStep ? "bg-destructive border-destructive scale-110" : "bg-card border-border/40")}>
                            <span className="text-[10px] font-black text-white">{idx + 1}</span>
                          </div>
                          <div className="glass-card p-6 rounded-2xl">
                             <h4 className="text-[10px] font-black uppercase tracking-widest mb-2">{step.name}</h4>
                             <p className="text-[11px] font-mono opacity-60 break-all bg-accent p-3 rounded-lg border border-border/40">{step.val}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-4 pt-12 border-t border-border/40">
                     <Button 
                        onClick={() => handleReplayAttack(selectedRequest)}
                        disabled={replayingId === selectedRequest.id}
                        className="flex-1 bg-destructive hover:bg-destructive/90 font-black uppercase text-xs h-16 rounded-2xl shadow-lg shadow-destructive/20 text-white"
                     >
                     ...
                     <Button variant="outline" onClick={() => setSelectedRequest(null)} className="flex-1 font-black uppercase text-xs h-16 rounded-2xl border-border/40">
                       Dismiss Signal
                     </Button>
                  </div>

                  <div className="pt-8 text-center">
                    <p className="text-[8px] font-black uppercase tracking-widest opacity-30">ShieldCore Security Protocol SC-92</p>
                  </div>
                </div>
              </>
            )}
        ...
        {activeNotification && (
          <div className={cn(
            "fixed bottom-6 right-6 z-[6000] w-80 glass-card border-destructive/50 bg-card p-5 rounded-2xl shadow-2xl transition-all duration-500 overflow-hidden",
            ...
            <button 
              ...
              className="absolute top-3 right-3 p-1 opacity-40 hover:opacity-100 transition-colors z-10"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-black uppercase tracking-tighter truncate">{activeNotification.attackType}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 bg-accent p-3 rounded-xl border border-border/40 mb-4">
              <div>
                <p className="text-[8px] font-black uppercase opacity-40 mb-0.5">Origin IP</p>
                <p className="text-[11px] font-mono opacity-80">{activeNotification.ip}</p>
              </div>
              <div>
                <p className="text-[8px] font-black uppercase opacity-40 mb-0.5">Confidence</p>
